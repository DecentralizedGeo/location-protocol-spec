...
- `location_type`: An identifier for the format of the `location` data (e.g., `coordinate_decimal`, `geojson`).
...
- **Location Types**: New `location_type` identifiers can be proposed to support different geospatial data formats beyond the core set (e.g., `h3`, `wkt`, `geohash`).
